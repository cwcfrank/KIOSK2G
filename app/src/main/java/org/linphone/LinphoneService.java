...
//	private boolean mTestDelayElapsed; // add a timer for testing
	...
	*/
	...
	public void onCreate() {
		...
		// In case restart after a crash. Main in SmartPhoneProxyActivity
...
*/
		LinphoneManager.getLc().addListener(mListener = new LinphoneCoreListenerBase() {
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				android.util.Log.d("sipDebug0507","call state message = "+message );
				if (instance == null) {
				...
				if (state == State.CallEnd && call.getCallLog().getStatus() == CallStatus.Missed) {
					...
					if (missedCallCount > 1) {
						body = getString(R.string.missed_calls_notif_title).replace("%i", String.valueOf(missedCallCount));
					} else {
...
//					return;
//				}
				android.util.Log.d("sipDebug0507","registration state message = "+smessage );
                Log.i(TAG, smessage);
	...
	private synchronized void setIncallIcon(IncallIconState state) {
		...
		Uri pictureUri = null;//==contact != null ? contact.getPhotoUri() : null;
		Bitmap bm = null;
		try {
...
*/
	...
	public void displayMessageNotification(String fromSipUri, String fromName, String message) {
		/*==
		Intent notifIntent = new Intent(this, SmartPhoneProxyActivity.class);
		...
		Uri pictureUri = null;
		try {
			LinphoneContact contact = null;//==ContactsManager.getInstance().findContactFromAddress(LinphoneCoreFactory.instance().createLinphoneAddress(fromSipUri));
			if (contact != null)
...
/*==
	private Class<? extends Activity> incomingReceivedActivity = SmartPhoneIncomingCallActivity.class;
*/
